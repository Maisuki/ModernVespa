...
$(window).mouseover(function (e) {
    if ($(e.target).hasClass('actions') || ($(e.target).is('a') && $(e.target).parent().hasClass('actions'))) {
        $(e.target).css('background', 'transparent');
        $(e.target).children('a').css('color', '#a6eb14');
    }
    else {
        $('.actions').css('background', '#a6eb14');
        $('.actions a').css('color', '#000');
    }
});
...
//		startheight:1330,
  ...
  $('.block-benefits-2').on('click', '.tabs a', function()
	...
	$('.block-clients .carousel').imagesLoaded(function()
	{
        ...
        $('.block-clients .carousel1').imagesLoaded(function()
	{
		$('.block-clients .carousel1').owlCarousel(
		{
                    navigation: true,
                    navigationText: ['<i class="fa fa-angle-left">', '<i class="fa fa-angle-right">'],
                    pagination: false,
