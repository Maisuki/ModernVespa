...
function load_latest_10() {
    $.ajax({
        ...
        complete: function (e, xhr, settings) {
            var data = ($.parseJSON(e.responseText));
            if (e.status === 200) {
                products10 = data.products;
                //alert("products10 : "+products10.length);
                for (var i = 0; i < 10; i++) {
                    
                    var product = products10[i];
                    //alert("product ("+ i  + ") : "+ product.name.toString());
                    var pId = product._id;
...
//                        var rating = (products[i].rating / 5) * 100;
                        ...
                        $("#top10productGridDesktop").append(outputDesktop);
                        /**** DESKTOP VERSION ****/
